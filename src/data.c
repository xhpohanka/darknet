...
matrix load_image_augment_paths(char **paths, int c, int n, int min, int max, int size, float angle, float aspect, float hue, float saturation, float exposure, int center)
{
    int i;
    matrix X;
    X.rows = n;
...
void correct_boxes(box_label *boxes, int n, float dx, float dy, float sx, float sy, int flip, float phi)
{
    int i;
    for(i = 0; i < n; ++i){
        ...
        if (phi != 0) {
            float cx = 0.5;
            float cy = 0.5;
            boxes[i].x = (boxes[i].x - cx) * cos(phi) - (boxes[i].y - cy) * sin(phi) + cx;
...
static void replace_label_path(char *path, const char *labeldir, char *labelpath)
{
    find_replace(path, "JPEGImages", labeldir, labelpath);
    find_replace(labelpath, "PNGImages", labeldir, labelpath);
    find_replace(labelpath, "SrcImages", labeldir, labelpath);
    find_replace(labelpath, "raw", labeldir, labelpath);
...
void fill_truth_swag(char *path, const char *labeldir, float *truth, int classes, int flip, float dx, float dy, float sx, float sy)
{
    char labelpath[4096];
    ...
    correct_boxes(boxes, count, dx, dy, sx, sy, flip, 0);
    float x,y,w,h;
    int id;
    int i;
...
void fill_truth_region(char *path, const char *labeldir, float *truth, int classes, int num_boxes, int flip, float dx, float dy, float sx, float sy)
{
    char labelpath[4096];
    replace_label_path(path, labeldir, labelpath);
...
void fill_truth_detection(char *path, const char *labeldir, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy, float phi)
{
    char labelpath[4096];
    replace_label_path(path, labeldir, labelpath);
    ...
    box_label *boxes = read_boxes(labelpath, &count);
    randomize_boxes(boxes, count);
    correct_boxes(boxes, count, dx, dy, sx, sy, flip, phi);
...
data load_data_region(int n, char **paths, const char *labeldir, int m, int w, int h, int size, int classes,
        float jitter, float hue, float saturation, float exposure)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        fill_truth_region(random_paths[i], labeldir, d.y.vals[i], classes, size, flip, dx, dy, 1./sx, 1./sy);
...
data load_data_swag(char **paths, const char *labeldir, int n, int classes, float jitter)
{
    int index = rand()%n;
    ...
    fill_truth_swag(random_path, labeldir, d.y.vals[0], classes, flip, dx, dy, 1./sx, 1./sy);
        ...
        float jitter, float angle, float hue, float saturation, float exposure)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        float phi = rand_uniform(-TWO_PI/360 * angle, TWO_PI/360 * angle);
        image rot = rotate_image(sized, phi, 0.5);

        random_distort_image(rot, hue, saturation, exposure);
        ...
        fill_truth_detection(random_paths[i], labeldir, boxes, d.y.vals[i], classes, flip, -dx/w, -dy/h, nw/w, nh/h, -phi);
...
void *load_thread(void *ptr)
{
    ...
    if (a.type == OLD_CLASSIFICATION_DATA){
        ...
        *a.d = load_data_regression(a.paths, a.n, a.m, a.c, a.classes, a.min, a.max, a.size, a.angle, a.aspect, a.hue, a.saturation, a.exposure);
    } else if (a.type == CLASSIFICATION_DATA){
        *a.d = load_data_augment(a.paths, a.n, a.m, a.c, a.labels, a.classes, a.hierarchy, a.min, a.max, a.size, a.angle, a.aspect, a.hue, a.saturation, a.exposure, a.center);
        ...
        *a.d = load_data_tag(a.paths, a.n, a.m, a.c, a.classes, a.min, a.max, a.size, a.angle, a.aspect, a.hue, a.saturation, a.exposure);
    }
    free(ptr);
    return 0;
}
...
data load_data_regression(char **paths, int n, int m, int c, int k, int min, int max, int size, float angle, float aspect, float hue, float saturation, float exposure)
{
    if(m) paths = get_random_paths(paths, n, m);
    data d = {0};
    d.shallow = 0;
    d.X = load_image_augment_paths(paths, c, n, min, max, size, angle, aspect, hue, saturation, exposure, 0);
...
data load_data_augment(char **paths, int n, int m, int c, char **labels, int k, tree *hierarchy, int min, int max, int size, float angle, float aspect, float hue, float saturation, float exposure, int center)
{
    if(m) paths = get_random_paths(paths, n, m);
    data d = {0};
    d.shallow = 0;
    d.w=size;
    d.h=size;
    d.X = load_image_augment_paths(paths, c, n, min, max, size, angle, aspect, hue, saturation, exposure, center);
    d.y = load_labels_paths(paths, n, labels, k, hierarchy);
    if(m) free(paths);
    return d;
}

data load_data_tag(char **paths, int n, int m, int c, int k, int min, int max, int size, float angle, float aspect, float hue, float saturation, float exposure)
{
    if(m) paths = get_random_paths(paths, n, m);
    data d = {0};
    d.w = size;
